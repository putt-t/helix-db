...
pub fn node_connections_inner(input: &HandlerInput) -> Result<protocol::Response, GraphError> {
    ...
    let node_id_str = if !input.request.body.is_empty() {
        match sonic_rs::from_slice::<sonic_rs::Value>(&input.request.body) {
            Ok(params) => params.get("node_id").and_then(|v| v.as_str()).map(|s| s.to_string()),
            Err(_) => None,
        }
    } else {
        None
    ...
    let node_id_str = node_id_str.ok_or_else(|| GraphError::New("node_id is required".to_string()))?;
    ...
    let node_id = if let Ok(uuid) = uuid::Uuid::parse_str(&node_id_str) {
        ...
        return Err(GraphError::New("Invalid node_id format - must be UUID or u128".to_string()));
    };
    ...
    let incoming_edges = db.in_edges_db.prefix_iter(&txn, &node_id.to_be_bytes())?
        .map(|result| {
            let (_, value) = result?;
    ...
    let outgoing_edges = db.out_edges_db.prefix_iter(&txn, &node_id.to_be_bytes())?
        .map(|result| {
            let (_, value) = result?;
